...
class App extends React.Component {
  ...
  bootClick = () => {
    ...
    let vidLink = 'qHFssXdRGQM';  //need to find out where 'data-id' is located in API<<<<<<
    ...
    return (
      <div className="App">
        ...
        {this.state.movies.map((movie) => {
            return <div key={movie.title}><img className="imgs" src={imPath + movie.poster_path} alt={movie.overview} id={movie.title} onClick={this.handleClick}/><h5 className="tiles"> {movie.title} <br/> ({movie.vote_average}/10)</h5></div>;
          })
        }
